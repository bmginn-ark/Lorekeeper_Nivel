...
@section('admin-title')
    {{ $category->id ? __('Edit') : __('Create') }} {{ __('Trait Category') }}
@endsection

@section('admin-content')
    {!! breadcrumbs([
        __('Admin Panel') => 'admin',
        __('Trait Categories') => 'admin/data/trait-categories',
        ($category->id ? __('Edit') : __('Create')) . ' ' . __('Category') => $category->id ? 'admin/data/trait-categories/edit/' . $category->id : 'admin/data/trait-categories/create',
    ]) !!}

    <h1>{{ $category->id ? __('Edit') : __('Create') }} {{ __('Trait Category') }}
        @if ($category->id)
    ...
    <div class="form-group">
        ...
        {!! Form::label('is_visible', __('Is Visible'), ['class' => 'form-check-label ml-3']) !!} {!! add_help(__('If turned off, the category will not be visible in the category list or available for selection in search. Permissioned staff will still be able to add traits to them, however.')) !!}
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('.delete-category-button').on('click', function(e) {
                e.preventDefault();
                loadModal("{{ url('admin/data/trait-categories/delete') }}/{{ $category->id }}", '{{ __("Delete Category") }}');
            });
        });
    </script>
@endsection
