<div class="card mb-3 stock {{ $stock ? '' : 'hide' }}">
    <div class="card-body">
        <div class="text-right mb-3"><a href="#" class="remove-stock-button btn btn-danger">{{ __('Remove') }}</a></div>
        <div class="form-group">
            {!! Form::label('item_id[' . $key . ']', __('Item')) !!}
            {!! Form::select('item_id[' . $key . ']', $items, $stock ? $stock->item_id : null, ['class' => 'form-control stock-field', 'data-name' => 'item_id']) !!}
        </div>
        <div class="form-group">
            {!! Form::label('cost[' . $key . ']', __('Cost')) !!}
            <div class="row">
                <div class="col-4">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    {!! Form::label('use_user_bank[' . $key . ']', __('Use User Bank'), ['class' => 'form-check-label ml-3']) !!} {!! add_help(__('This will allow users to purchase the item using the currency in their accounts, provided that users can own that currency.')) !!}
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group mb-0">
                    ...
                    {!! Form::label('use_character_bank[' . $key . ']', __('Use Character Bank'), ['class' => 'form-check-label ml-3']) !!} {!! add_help(__('This will allow users to purchase the item using the currency belonging to characters they own, provided that characters can own that currency.')) !!}
                </div>
            </div>
        </div>
        <div class="form-group">
            ...
            {!! Form::label('is_limited_stock[' . $key . ']', __('Set Limited Stock'), ['class' => 'form-check-label ml-3']) !!} {!! add_help(__('If turned on, will limit the amount purchaseable to the quantity set below.')) !!}
        </div>
        <div class="card mb-3 stock-limited-quantity {{ $stock && $stock->is_limited_stock ? '' : 'hide' }}">
            <div class="card-body">
                <div>
                    {!! Form::label('quantity[' . $key . ']', __('Quantity')) !!} {!! add_help(__('If left blank, will be set to 0 (sold out).')) !!}
                    {!! Form::text('quantity[' . $key . ']', $stock ? $stock->quantity : 0, ['class' => 'form-control stock-field', 'data-name' => 'quantity']) !!}
                </div>
            </div>
        </div>
        <div>
            {!! Form::label('purchase_limit[' . $key . ']', __('User Purchase Limit')) !!} {!! add_help(__('This is the maximum amount of this item a user can purchase from this shop. Set to 0 to allow infinite purchases.')) !!}
