@if ($trade)
    @if ($trade->sender_id == Auth::user()->id)
        ...
        <div class="text-right">
            {!! Form::submit($trade->is_sender_confirmed ? '미승인' : '승인', ['class' => 'btn btn-' . ($trade->is_sender_confirmed ? 'danger' : 'primary')]) !!}
        </div>
        {!! Form::close() !!}
    @else
        ...
        {!! Form::open(['url' => 'trades/' . $trade->id . '/confirm-offer']) !!}
        <div class="text-right">
            {!! Form::submit($trade->is_recipient_confirmed ? '미승인' : '승인', ['class' => 'btn btn-' . ($trade->is_recipient_confirmed ? 'danger' : 'primary')]) !!}
        </div>
        {!! Form::close() !!}
    @endif
@else
