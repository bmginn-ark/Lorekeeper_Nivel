    ...
    */
    ...
    public function getCharacters(Request $request) {
        ...
        return view('browse.masterlist', [
            ...
            'categories'  => [0 => __('Any Category')] + CharacterCategory::whereNotIn('id', $subCategories)->visible(Auth::check() ? Auth::user() : null)->orderBy('character_categories.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'specieses'   => [0 => __('Any Species')] + Species::whereNotIn('id', $subSpecies)->visible(Auth::check() ? Auth::user() : null)->orderBy('specieses.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'subtypes'    => [0 => __('Any Subtype')] + Subtype::visible(Auth::check() ? Auth::user() : null)->orderBy('subtypes.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'rarities'    => [0 => __('Any Rarity')] + Rarity::orderBy('rarities.sort', 'DESC')->pluck('name', 'id')->toArray(),
    ...
    public function getMyos(Request $request) {
        ...
        return view('browse.myo_masterlist', [
            ...
            'specieses'   => [0 => __('Any Species')] + Species::visible(Auth::check() ? Auth::user() : null)->orderBy('specieses.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'rarities'    => [0 => __('Any Rarity')] + Rarity::orderBy('rarities.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'features'    => Feature::getDropdownItems(),
            'sublists'    => Sublist::orderBy('sort', 'DESC')->get(),
            'userOptions' => User::query()->orderBy('name')->pluck('name', 'id')->toArray(),
    ...
    public function getSublist(Request $request, $key) {
        ...
        return view('browse.sub_masterlist', [
            'isMyo'       => false,
            'characters'  => $query->paginate(24)->appends($request->query()),
            'categories'  => [0 => __('Any Category')] + $subCategory,
            'specieses'   => [0 => __('Any Species')] + $subSpecies,
            'subtypes'    => [0 => __('Any Subtype')] + Subtype::visible(Auth::check() ? Auth::user() : null)->orderBy('subtypes.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'rarities'    => [0 => __('Any Rarity')] + Rarity::orderBy('rarities.sort', 'DESC')->pluck('name', 'id')->toArray(),
            'features'    => Feature::getDropdownItems(),
            'sublist'     => $sublist,
