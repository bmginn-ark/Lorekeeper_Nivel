...
- **aish-scales.com**: 라이브 서버용 원격 (ssh://root@aish-scales.com/var/aish-scales.com/site_hub.git)
...
- 라이브 반영 → `git push aish-scales.com main`
...
**이미 연동되어 있습니다.**  
`aish-scales.com` 원격이 라이브 서버의 `site_hub.git`을 가리키고 있으므로:

- **로컬에서** `git push aish-scales.com main` 하면 → 코드가 라이브 서버 저장소로 올라갑니다.
- **라이브 서버에서** 실제 사이트 디렉터리(예: `/var/www/...`)가 이 저장소를 **clone** 했거나 **pull** 하도록 되어 있다면, 서버 쪽에서 다음만 하면 됩니다:

  ```bash
  cd /var/aish-scales.com/사이트디렉터리   # 실제 경로로 변경
  git pull
...
| 2 | `d:\00\Lorekeeper_Nivel` 에서 `git add -A` → `git commit` → `git push aish-scales.com main` |
| 3 | (선택) 본인 GitHub 저장소 추가 후 `git push mygithub main` 으로 백업 |
| 4 | 라이브 서버는 이미 `aish-scales.com` 원격으로 연동 가능; 서버에서는 `git pull` 로 반영 |
