...
@section('design-content')
    ...
    @if ($request->status == 'Draft' && $request->user_id == Auth::user()->id && $request->character)
        <p>요청에 추가할 항목 및/또는 재화를 선택하세요. These items will be removed from your inventory{{ $request->character->is_myo_slot ? '' : ' and/or character' }} but refunded if removed from the request, the request is rejected, or the
